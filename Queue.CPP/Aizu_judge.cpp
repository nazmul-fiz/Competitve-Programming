            ...
            *                                                                                     *
            |                                 Nazmul_Fiz                                          |
            *                                 Date: 13/ 1/ 26                                     *
            |                       Topic/ Problem in Aizu Judge :    ALDS1_3_B
            **************************************************************************************/ /*
...
e4 {
  ...
  queue<pair<string, int>>Robin_Carp;
  
  for(int i=0; i<n; i++)
  {
    string name; int tim;
    cin>>name>>tim;
    Robin_Carp.push({name, tim});
  }
  
  
  ll ache= 0;
  
  while(!Robin_Carp.empty())
  {
    auto [name, rem]= Robin_Carp.front();
    Robin_Carp.pop();
