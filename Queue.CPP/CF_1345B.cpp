...
e4{
  ...
  ll big_num=1000000001LL;
  
  vecll Robin_carp_cnt;
  
  for(ll i=0; ; i++)
  {
    ll formula= (3*i*i +i)/2;
    if(formula > big_num)
    {
      break;
    }
    Robin_carp_cnt.push_back(formula);
  }
  // precompute done with The formula To build a pyramid wih Best Height //
  ...
  while(t--)
  {
    ...
    while(n>= 2)
    {
      ll banano_possible= upper_bound(Robin_carp_cnt.begin(), Robin_carp_cnt.end(), n) - Robin_carp_cnt.begin() - 1;
       if(banano_possible < 0)
       {
         break;
       }
       n-= Robin_carp_cnt[banano_possible];
