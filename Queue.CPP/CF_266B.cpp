...
e4 
{
  ...
  queue<char>Robin_Carp_string;
  bool geeks= false;
  
  for(char geeks: s) 
  {
    Robin_Carp_string.push(geeks);
  }
  ...
  while(t--)
  {
    ...
    while(!Robin_Carp_string.empty())
    {
      char fi= Robin_Carp_string.front();
      Robin_Carp_string.pop();
      
      if(fi == 'B' and Robin_Carp_string.front()== 'G')
      {
        Main_Q.push(g);
        Main_Q.push(c);
    ...
    Robin_Carp_string= Main_Q;
    
  }
  while(!Robin_Carp_string.empty())
  {
    cout<<Robin_Carp_string.front();
    Robin_Carp_string.pop();
