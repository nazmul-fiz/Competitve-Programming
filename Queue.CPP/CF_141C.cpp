            ...
            *                                                                                     *
            |                                 Nazmul_Fiz                                          |
            *                                 Date: 20/ 1/ 26                                     *
            |                      Topic/ Approach: Solving Marathon Contest 2026
            **************************************************************************************/ /*
...
//
// Upsolved with Tons of Explain, and its Deque based on Line Height , and line maintain //
bool Feasibility_Compare(pair<string, int> &a, pair<string, int> &b)
{
...
e4
{
    ...
    vector<pair<string, int>> Geeks_People(n);

    for(int i = 0; i < n; i++)
    {
        cin>> Geeks_People[i].first >> Geeks_People[i].second;
    }


    sort(Geeks_People.begin(), Geeks_People.end(), Feasibility_Compare);

    map<int, stack<string>> Geeks_nam;
    for(auto &x : Geeks_People)
        Geeks_nam[x.second].push(x.first);


    vector<pair<int,int>> Geeks_Sajano(n);

    int cur= 3001;
    int have= 1;


    if(Geeks_People[0].second > 0)
    {
        cout<< -1 <<endl;
        return 0;
    }

    Geeks_Sajano[0]={3001, 0};
    for(int i=1; i< n; i++)
    {
        int k = Geeks_People[i].second;
        ...
        for(int j= n-2; j >= k; j--)
        {
            Geeks_Sajano[j +1] = Geeks_Sajano[j];
        }

        if(k != 0)
        {
            cur--;
            Geeks_Sajano[k] = {cur, k};
        }
        else
        {
            Geeks_Sajano[0] = {3001, 0};
    ...
    for(int i = 0; i < n; i++)
    {
        int k = Geeks_Sajano[i].second;
        cout<< Geeks_nam[k].top() << " "<< Geeks_Sajano[i].first <<endl;
       Geeks_nam[k].pop();
    }
