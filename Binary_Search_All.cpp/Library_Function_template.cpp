...
e4
{
    ...
    for (int i = 0; i < N; i++) {
        int L = minLen + (i % (maxLen - minLen + 1));
        ...
        while ((int)password.size() < L) {
            password.push_back(lower[li]);
            if ((int)password.size() == L) break;
            password.push_back(upper[ui]);
            if ((int)password.size() == L) break;
            password.push_back(digits[di]);
            if ((int)password.size() == L) break;

            password.push_back(P[pi]);
            if ((int)password.size() == L) break;
