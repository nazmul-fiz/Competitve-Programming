...
void delete_at_position(node *head, int pos)
{
    ...
    for(int i=1; i<=pos-1; i++)
    {
        ...
        if(tmp == NULL)
        {
            cout<<endl<<"Invalid delete position"<<endl<<endl;
            return ;
        }
    }
     if(tmp->next == NULL)
...
void delete_head(node *&head)
{
  if(head == NULL)
        {
            cout<<endl<<"Invalid delete Head"<<endl<<endl;
            return;
        }
...
int main() 
{
   ...
   // here we chose option and to continue the loop to Insert linked list
   {
    ...
    cout <<"Option 4: Insert At Head: "<< endl; //error handled
    cout <<"Option 5: Delete from position: "<< endl; //error handled
    cout <<"Option 6: Delete Head: "<< endl; // error handled
    cout <<"Option 7: I want to Off: "<< endl; //error handled
    int op;
    cin>>op;
    if (op==1)
    {
